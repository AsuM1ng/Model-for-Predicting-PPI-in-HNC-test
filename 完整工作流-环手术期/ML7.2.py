...
def calc_metrics(y, prob, t):
    ...
    # 防退化：若预测全为同一类，confusion_matrix.ravel() 只有1个值
    cm = confusion_matrix(y, pred)
    if cm.size == 1:
        tn = cm[0, 0] if y.sum() == 0 else 0
        fp = 0 if y.sum() == 0 else cm[0, 0]
        fn = 0 if y.sum() > 0 else cm[0, 0]
        tp = cm[0, 0] if y.sum() > 0 else 0
    else:
...
# ================== SHAP ==================
def plot_shap(model_or_pipe, X, name, is_pipeline=False):
    """
    is_pipeline: True means model_or_pipe is a sklearn Pipeline,
                  extract the last step as the classifier.
    """

    try:
        # --- 提取真正的模型 ---
        if is_pipeline:
            clf = model_or_pipe.named_steps["classifier"]
        else:
            clf = model_or_pipe

        # --- 处理不同模型类型 ---
        if hasattr(clf, "predict_proba"):
            # Tree-based models (RF, XGB) → 用 TreeExplainer
            if hasattr(clf, "feature_importances_") or type(clf).__name__ in ("RandomForestClassifier", "XGBClassifier"):
                explainer = shap.Explainer(clf, X)
                shap_values = explainer(X)
                exp_type = "tree"
            else:
                # 其他有 predict_proba 的模型用 KernelExplainer (linear)
                explainer = shap.KernelExplainer(
                    lambda x: clf.predict_proba(x)[:, 1], X, algorithm="linear"
                )
                shap_values = explainer(X)
                exp_type = "kernel_linear"
        else:
            raise ValueError(f"Model {type(clf).__name__} has no predict_proba")
        ...
        if exp_type == "tree":
            ...
            # KernelExplainer 返回 Explanation 对象
        ...
        plt.figure()
        shap.summary_plot(
            shap_values.values if exp_type == "tree" else shap_values.values,
            X,
            plot_type="bar",
            show=False
        )
        ...
        print(f"  SHAP failed for {name} → using fallback: {e}")
        _plot_shap_fallback(clf, X, name, is_pipeline)


def _plot_shap_fallback(clf, X, name, is_pipeline):
    """Fallback: 使用 permutation importance 或均值绝对值近似 SHAP bar"""
    try:
        ...
        plt.savefig(OUT / f"SHAP_bar_{name}.png", bbox_inches="tight")
        plt.close()
        print(f"  Fallback bar plot saved for {name}")

    except Exception as e2:
        print(f"  Fallback also failed for {name}: {e2}")
        # 最后保底：用模型自己的 feature_importances_（如果有）
        if hasattr(clf, "feature_importances_"):
            ...
            plt.savefig(OUT / f"SHAP_bar_{name}.png", bbox_inches="tight")
            plt.close()
            print(f"  Final fallback (feature_importances_) saved for {name}")
...
def train_all(Xtr, ytr, Xte, yte):
    ...
    for name in models:
        ...
        plot_shap(best, Xtr, name, is_pipeline=True)
    ...
    results["XGB"] = res
    plot_shap(best, Xtr, "XGB", is_pipeline=False)
