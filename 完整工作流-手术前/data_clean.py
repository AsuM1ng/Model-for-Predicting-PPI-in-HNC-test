...
- 输出三个文件：
  1) `data1.csv`：清洗后的数据（保存在脚本根目录）；
  2) `outputs/data_clean/category_mappings.json`：多分类/二分类字段编码字典；
  3) `outputs/data_clean/data_review_summary.txt`：处理前数据审阅摘要。
"""
...
WRITE_PATH = Path("data1.csv")           # data1.csv 保持在根目录，不改
